...
Cada encuesta tiene un `owner` relacionado con `Users`. Las encuestas creadas desde Payload se asignan automáticamente al usuario que las crea. La encuesta demo `experiencia` se asigna al primer usuario cuando se ejecuta `npm run seed` después de crear ese usuario. Los usuarios con rol `editor` solo pueden consultar sus encuestas; los usuarios `admin` también pueden consultar encuestas legacy creadas antes de añadir `owner`.
...
No guardes el token ni la API key en el repositorio. La IA respeta el alcance del usuario: los editores solo consultan encuestas cuyo `owner` coincide con ellos; los administradores pueden consultar también encuestas legacy sin propietario.
